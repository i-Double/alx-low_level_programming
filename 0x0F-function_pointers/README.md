...
You can assume num1 and num2 are integers, so use the
atoi function to convert them from the string input to int
...
by a new line, and exit with the status 99
if the user tries to divide (/ or %) by 0, print Error, followed by a n
ew line, and exit with the status 100
...
to perform the operation asked by the user. You’re
...
to the operator given as a parameter. Example: get_op_func("+") 
...
Error, followed by a new line, and exit with the status
2
